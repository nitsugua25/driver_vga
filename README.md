...
```c
static uint16_t vga_buffer[25][80];
```
...
```c
static uint8_t framebuffer[480][640][3];
```
...
### Main Function: `vga_write(int row, int col, const char *txt)`
```c
vga_write(5, 10, "HELLO");  // Write "HELLO" at row 5, column 10
```

**What it does**:
1. Takes each character in the text
2. Checks if it's A-Z or space (reject others)
3. Stores it in vga_buffer with color
...
.global _start
_start:
    movl $stack_top, %esp    # Set up stack pointer
...
3. **call kernel_main**: Jumps to our C function `kernel_main()`
4. **cli/hlt**: If kernel_main returns (it shouldn't), disable interrupts and halt forever
...
for (bus = 0; bus < 256; bus++) {           // Try all buses
    for (slot = 0; slot < 32; slot++) {     // Try all slots
        vendor_id = pci_read_config(...);   // Read device ID
        if (vendor_id == 0xFFFF) continue;  // No device here
        
        class_code = pci_read_config(...);  // Read device type
        if (class_code == 0x0300) {         // VGA controller?
            bar0 = pci_read_config(...);    // Get framebuffer address
            return SUCCESS;
        }
    }
}
...
Sets up stack
    ↓
Calls kernel_main() (main.c)
    ↓
vga_init_text() - prepares VGA driver
    ↓
vga_clear() - fills screen with spaces
    ↓
vga_write_text() - writes colored text to 0xB8000
    ↓
pci_scan_vga() - scans PCI bus for VGA card
    ↓
...
make clean
make
qemu-system-i386 -cdrom vga_kernel.iso -display curses
...
Once the stack is ready, we can safely jump to C (`call kernel_main`).
...
- Verify with: `readelf -l vga_kernel.bin` (multiboot should be in first LOAD segment)

### Issue 2: Blank screen in QEMU
**Cause**: VGA memory not being written correctly.
**Solution**: 
- Verify address: `#define VGA_MEMORY 0xB8000`
- Test with simple code: `*(uint16_t*)0xB8000 = 0x0F41;` (white 'A')
...
```bash
# Simulation
cd simulation && make && ./vga_sim

# Virtualization
cd virtualisation && make && qemu-system-i386 -cdrom vga_kernel.iso -display curses
```
...
- [x] Function `vga_write(row, col, txt)`
...
- **LD (Linker)**: https://ftp.gnu.org/old-gnu/Manuals/ld-2.9.1/html_mono/ld.html
